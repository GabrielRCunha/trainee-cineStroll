    ...
    <form id="formEditar<?= $usuario->id ?>" method="POST" action="/listaDeUsuarios/edit" enctype="multipart/form-data">
        ...
        <div class="modalInfo">
            <div class="modalInputs">
                <div class="modalNome">
                    <p>Nome:</p>
                    <input type="text" id="editarNome<?= $usuario->id ?>" value="<?= $usuario->nome ?>" placeholder=""
                        name="nome">
                    <p class="aviso" id="avisoNomeEditar<?= $usuario->id ?>">Nome é obrigatório</p>
                </div>
                <div class="modalEmail">
                    <p>Email:</p>
                    <input type="email" id="editarEmail<?= $usuario->id ?>" value="<?= $usuario->email ?>" placeholder=""
                        name="email">
                    <p class="aviso" id="avisoEmailEditar<?= $usuario->id ?>">Email é obrigatório</p>
                </div>
                <div class="modalSenha">
                    <p>Senha:</p>
                    <input type="password" id="editarSenha<?= $usuario->id ?>" value="<?= $usuario->senha ?>" placeholder=""
                        name="senha">
                    <i class="bi bi-eye" id="iconeEditarSenha<?= $usuario->id ?>"
                        onclick="mostrarSenha('iconeEditarSenha<?= $usuario->id ?>', 'editarSenha<?= $usuario->id ?>')"></i>
            ...
            <div class="modalFoto" id="fotoPerfilEditar<?= $usuario->id ?>">
                ...
                <input type="file" accept="image/jpeg, image/png, image/jpg" id="imgInputEditar<?= $usuario->id ?>"
                    style="display: none;" name="imagem">
                <button class="botaoEditarImagem" type="button"
                    onclick="document.getElementById('imgInputEditar<?= $usuario->id ?>').click()">Editar imagem</button>
        ...
        <div class="botoes-excluir">
            <button class="botaoFecharModalEditar" type="button"
                onclick="fecharModal('editar<?= $usuario->id ?>', event )">Cancelar</button>
